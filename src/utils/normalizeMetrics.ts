...
export type NormalizedResult = Omit<AggregatedResult, 'less_spent_at' | 'big_spent_at' | 'total_spend_galactic' | 'rows_affected' | 'less_spent_value' | 'big_spent_value' | 'average_spend_galactic'> & {
...
export const normalizeMetrics = (data: Partial<AggregatedResult>): NormalizedResult => ({
    total_spend_galactic: Intl.NumberFormat('ru-RU').format((Math.round(data.total_spend_galactic ?? 0))),
    ...
    average_spend_galactic: Intl.NumberFormat('ru-RU').format(Math.round(data.average_spend_galactic ?? 0)),
